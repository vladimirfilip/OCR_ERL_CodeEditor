...
class TestSymTable(TestCase):
    ...
    def test_sub_table_from(self):
        sub_tbl: SymTable = self.sym_table.sub_table_from('x', True)
        self.assertIsNotNone(sub_tbl)
        self.assertIs(self.sym_table, sub_tbl.parent)
        self.assertIsNone(sub_tbl.sub_table_from('y'))

    def test_sub_table_of(self):
        names = ['a', 'b', 'c']
        sub_tbl = self.sym_table
        for n in names:
            sub_tbl = sub_tbl.sub_table_from(n, True)
        self.assertIs(sub_tbl, self.sym_table.sub_table_of(names))

    def test_with_sub_table_of(self):
        with self.sym_table.sub_table_from('x', True) as st:
            t = st
            self.assertIs(st, self.sym_table.sub_tables.pop('x'))
        self.assertIsNotNone(t)
        self.assertIsNone(t.parent)
        self.assertIs(t, t.root)

    def test_clear_sub_table(self):
        #
        # Clear one sub-table
        #
        t1 = self.sym_table.sub_table_from('x', True)
        t2 = self.sym_table.clear_sub_table('x')
        self.assertIs(t1, t2)
        self.assertEqual(0, len(self.sym_table.sub_tables))

        #
        # Clear all sub-tables
        #
        def sub_table(i: int) -> SymTable:
            result: SymTable = self.sym_table.sub_table_from(str(i), True)
            self.assertEqual(i, result.lookup_symbol("number", i))
            return result

        sts1 = sorted([sub_table(i) for i in range(0, 10)], key=lambda t: t.lookup_symbol("number"))
        sts2 = sorted(self.sym_table.clear_sub_table(), key=lambda t: t.lookup_symbol("number"))
        self.assertListEqual(sts1, sts2)
        self.assertEqual(0, len(self.sym_table.sub_tables))
    ...
    def test_lookup_deep_symbol(self):
        #
        # Happy path
        #
        names = ['a', 'b', 'c', 'd', 'e']
        t = self.sym_table
        for n in names[:-1]:
            t = t.sub_table_from(n, True)
        self.assertEqual(3, self.sym_table.lookup_deep_symbol(names, 3))
        self.assertEqual(3, t.lookup_symbol(names[-1]))
        #
        # Missing symbol
        #
        self.assertIsNone(self.sym_table.lookup_deep_symbol(names[:-1] + ['f']))
        #
        # Missing sub-table
        #
        self.assertIsNone(self.sym_table.lookup_deep_symbol(names[:-2] + ['e']))

    def test_update_deep_symbol(self):
        names = ['a', 'b', 'c', 'd', 'e']
        t = self.sym_table
        for n in names[:-1]:
            t = t.sub_table_from(n, True)
        self.assertTrue(self.sym_table.update_deep_symbol(names, 3))
        self.assertEqual(3, self.sym_table.lookup_deep_symbol(names))
        self.assertTrue(self.sym_table.update_deep_symbol(names, 4))
        self.assertEqual(4, self.sym_table.lookup_deep_symbol(names))
        self.assertTrue(self.sym_table.update_deep_symbol(names, None))
        self.assertIsNone(self.sym_table.lookup_deep_symbol(names))
        wrong_names = names[:-2] + ['e', 'f']
        self.assertFalse(self.sym_table.update_deep_symbol(wrong_names))
